...
static void *ini_stbds_realloc(void *context, void *ptr, size_t size) {
  (void)context;
  return ptr ? ini_realloc(ptr, size) : ini_malloc(size);
}

#define STBDS_REALLOC(context, ptr, size)                                      \
  ini_stbds_realloc((context), (ptr), (size))
#define STBDS_FREE(context, ptr) ((void)(context), ini_free(ptr))
#else
#include <stdlib.h>
#endif
...
#if !INI_USE_STACK
  stbds_arrsetcap(line, INI_INITIAL_ALLOC);
  if (!line) {
    return -2;
  }
#endif
